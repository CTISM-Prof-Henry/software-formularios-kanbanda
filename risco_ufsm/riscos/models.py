...
class PlanoDeRisco(models.Model):
    ...
    criado_por      = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.PROTECT, related_name='planos_criados')
    status          = models.CharField(max_length=20, choices=STATUS_CHOICES, default=STATUS_SEM_TRATAMENTO, db_index=True)
...
class IdentificacaoRisco(models.Model):
    ...
    plano           = models.OneToOneField(PlanoDeRisco, on_delete=models.CASCADE, related_name='identificacao')
    tipologia       = models.CharField(max_length=20, choices=TIPOLOGIA_CHOICES)
    macroprocesso   = models.ForeignKey(Macroprocesso, on_delete=models.SET_NULL, null=True, blank=True)
    objetivo_pdi    = models.ForeignKey(ObjetivoPDI, on_delete=models.SET_NULL, null=True, blank=True)
    descricao_evento = models.TextField('Descrição do Evento de Risco')
    causas          = models.TextField('Causas')
    consequencias   = models.TextField('Conseqüências')
...
class AvaliacaoRisco(models.Model):
    ...
    plano               = models.OneToOneField(PlanoDeRisco, on_delete=models.CASCADE, related_name='avaliacao')
    ...
    nivel_inerente      = models.CharField(max_length=10, choices=NIVEL_CHOICES, editable=False, blank=True)
    ...
    nivel_residual      = models.CharField(max_length=10, choices=NIVEL_CHOICES, editable=False, blank=True)
    ...
    def save(self, *args, **kwargs):
        ...
        self.risco_residual = round(self.risco_inerente * factor, 2) if 'round' in dir() else round(self.risco_inerente * fator, 2)
...
class TratamentoRisco(models.Model):
    ...
    plano                   = models.OneToOneField(PlanoDeRisco, on_delete=models.CASCADE, related_name='tratamento')
    ...
    situacao                = models.CharField(max_length=15, choices=SITUACAO_CHOICES, default='NAO_INICIADO')
